...
# ============================================================================
# MLPs & Small Blocks matching checkpoint keys
# ============================================================================
...
class TrackerTwoWayAttentionBlock(nn.Module):
    def __init__(self, hidden_size: int = 256, num_heads: int = 8, mlp_dim: int = 2048):
        super().__init__()
        self.self_attn = Sam3Attention(hidden_size, num_heads)
        self.layer_norm1 = nn.LayerNorm(hidden_size)

        self.cross_attn_token_to_image = Sam3Attention(hidden_size, num_heads)
        self.layer_norm2 = nn.LayerNorm(hidden_size)

        self.mlp = TrackerMLP(hidden_size, mlp_dim, hidden_size, num_layers=0)
        self.layer_norm3 = nn.LayerNorm(hidden_size)

        self.cross_attn_image_to_token = Sam3Attention(hidden_size, num_heads)
...
class TrackerTwoWayTransformer(nn.Module):
    def __init__(self, hidden_size: int = 256, num_layers: int = 2, num_heads: int = 8):
        super().__init__()
        self.layers = nn.ModuleList([
            TrackerTwoWayAttentionBlock(hidden_size, num_heads) for _ in range(num_layers)
        ])
        self.final_attn_token_to_image = Sam3Attention(hidden_size, num_heads)
...
class TrackerMaskDecoder(nn.Module):
    def __init__(self, hidden_size: int = 256):
        super().__init__()
        self.transformer = TrackerTwoWayTransformer(hidden_size=hidden_size, num_layers=2)
...
class MemoryAttentionLayer(nn.Module):
    def __init__(self, hidden_size: int = 256, num_heads: int = 8):
        ...
        self.cross_attn_image = Sam3Attention(hidden_size, num_heads)
        self.layer_norm2 = nn.LayerNorm(hidden_size)
    ...
    def forward(self, x, memory):
        ...
        h = x + self.cross_attn_image(x, memory, memory)[0]
...
class TrackerMemoryAttention(nn.Module):
    def __init__(self, hidden_size: int = 256, num_layers: int = 4):
        super().__init__()
        self.layers = nn.ModuleList([
            MemoryAttentionLayer(hidden_size=hidden_size) for _ in range(num_layers)
        ])
...
class TrackerMemoryEncoder(nn.Module):
    def __init__(self, hidden_size: int = 256):
        ...
        self.mask_downsampler.layers = nn.ModuleList([
            MaskDownsamplerLayer(1 if i == 0 else 32 * (2**(i-1)), 32 * (2**i))
            for i in range(4)
        ])
...
class TrackerPromptEncoder(nn.Module):
    def __init__(self, hidden_size: int = 256):
        ...
        self.mask_embed.conv1 = nn.Conv2d(1, 16, kernel_size=2, stride=2)
        self.mask_embed.layer_norm1 = nn.LayerNorm(16)
        self.mask_embed.conv2 = nn.Conv2d(16, 64, kernel_size=2, stride=2)
        self.mask_embed.layer_norm2 = nn.LayerNorm(64)
        self.mask_embed.conv3 = nn.Conv2d(64, hidden_size, kernel_size=1)
        ...
        self.shared_embedding.positional_embedding = nn.Parameter(torch.zeros(1, hidden_size))
...
class Sam3TrackerModel(nn.Module):
    """
    Submodule matching all `tracker_model.*` checkpoint keys.
    """
    def __init__(self, hidden_size: int = 256):
        super().__init__()
        self.mask_decoder = TrackerMaskDecoder(hidden_size)
        self.mask_downsample = nn.Conv2d(1, 1, kernel_size=4, stride=4)
        self.memory_attention = TrackerMemoryAttention(hidden_size, num_layers=4)
        ...
        self.temporal_positional_encoding_projection_layer = nn.Linear(hidden_size, hidden_size)

        # Standalone parameter buffers
        self.register_parameter("memory_temporal_positional_encoding", nn.Parameter(torch.zeros(1, 16, hidden_size)))
        ...
        self.register_parameter("occlusion_spatial_embedding_parameter", nn.Parameter(torch.zeros(1, hidden_size)))

        self.shared_image_embedding = nn.Module()
        self.shared_image_embedding.positional_embedding = nn.Parameter(torch.zeros(1, hidden_size))